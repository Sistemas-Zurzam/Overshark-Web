            ...
            <div class="mb-9 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p class="mt-2 text-slate-500">
                        @if ($marca !== '' && $search !== '')
                            Resultados de {{ $marca }} para "{{ $search }}".
                        @elseif ($marca !== '')
                            Productos de la marca {{ $marca }}.
                        @elseif ($search !== '')
                            Resultados para "{{ $search }}".
                        @else
                            Escribe un producto o elige una marca.
                        @endif
                    </p>
                </div>
                ...
                <form action="{{ route('web.products.search') }}" method="GET" class="flex w-full max-w-sm items-center gap-2">
                    ...
                    <div class="relative w-full">
                        ...
                        <input type="text" id="simple-search" name="q" value="{{ $search }}" class="block w-full rounded-lg border border-slate-300 bg-[#F7F7F7] py-2.5 pl-9 pr-3 text-sm font-semibold text-slate-950 outline-none placeholder:text-slate-500 focus:border-cyan-600 focus:ring-4 focus:ring-cyan-100" placeholder="Buscar producto...">
                    </div>
            ...
            @if ($products->isEmpty())
                <div class="grid min-h-64 place-items-center rounded-3xl border border-dashed border-slate-200 bg-slate-50 px-5 text-center">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-400">Prueba otra búsqueda o elige una marca diferente.</p>
                    </div>
                </div>
            @else
                <div class="-mx-5 flex snap-x snap-mandatory gap-5 overflow-x-auto px-5 pb-4 sm:mx-0 sm:grid sm:grid-cols-2 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-4">
                    @foreach ($products as $product)
                        ...
                        <article class="group basis-[78vw] shrink-0 snap-start rounded-2xl bg-white p-4 shadow-xl shadow-slate-200/70 transition hover:-translate-y-1 sm:basis-auto">
                            ...
                            <div class="p-5">
                                <p class="mt-1 text-[11px] font-black uppercase tracking-[0.16em] text-cyan-700">{{ $product->marca ?: ($product->empresa_nombre ?? 'Overshark') }}</p>
